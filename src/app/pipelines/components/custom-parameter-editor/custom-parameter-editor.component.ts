...
export class CustomParameterEditorComponent implements OnInit, OnDestroy{
  ...
  selectedParam:PipelineStepParam;
  ...
  private getPipeline(step: PipelineStep) {
    ...
    if (value) {
      pipelineId = CustomParameterEditorComponent.getPipelineId(value);
    }
      ...
      if (param) {
        ...
        switch (typeof value) {
          ...
          case 'string':
            ...
            pipelineId = CustomParameterEditorComponent.getPipelineId(p);
            break;
        }
      }
    }
  ...
  private generateOutputs(step: PipelineStep) {
    ...
    outputs.push(CustomParameterEditorComponent.generateErrorOutput());
    return outputs;
  }
  ...
  private addNodeToPipeline(node, pipeline) {
    ...
    CustomParameterEditorComponent.adjustStepParameterType(step);
  ...
  get codeViewData() {
    return JSON.stringify(this.selectedParam, null, 4);
  }
}
