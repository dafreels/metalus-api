...
export class ExecutionEditorComponent {
  ...
  constructor(
    public dialogRef: MatDialogRef<ExecutionEditorComponent>,
  ...
  openPipelinePropertiesEditor(pipelineId: string) {
    ...
    let params = this.data.execution.pipelineParameters.find(
      (param) => param.pipelineId === pipelineId
    );
    if (!params) {
      params = {
        pipelineId,
  ...
  drop(event: CdkDragDrop<Pipeline[]>) {
  ...
  closeDialog() {
    ...
    this.selectedPipelines.forEach((pipeline) => {
      ...
      pipelineParameters.push(
        this.data.execution.pipelineParameters.find(
