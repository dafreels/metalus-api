...
class QuboleAWS extends BaseProvider {
  ...
  async getClusters(providerInstance, user) {
    ...
    const response = await client.get('https://us.qubole.com/api/v2.2/clusters');
    const clusterList = [];
    if (response.data && response.data.clusters && response.data.clusters.length > 0) {
      for await (let cluster of response.data.clusters) {
        ...
        clusterList.push({
          id: cluster.id,
          name: cluster.cluster_info && cluster.cluster_info.label ? cluster.cluster_info.label.join(',') : '',
          version: cluster.engine_config && cluster.engine_config.spark_settings && cluster.engine_config.spark_settings.spark_version ? cluster.engine_config.spark_settings.spark_version : '',
          state: instance.state,
          source: 'API',
          startTime: instance.start_at ? new Date(instance.start_at).getTime() : null,
          terminationTime: instance.down_at ? new Date(instance.down_at).getTime() : null
        });
      }
    }
    return clusterList;
  ...
  async getJob(providerInformation, providerInstance, user) {
    ...
    return {
      ...
      executionDuration: response.data.start_time && response.data.end_time ? response.data.end_time - response.data.start_time : 0
    };
  }
  ...
  async executeApplication(providerInstance, user, runConfig) {
    ...
    const logOptions = runConfig.logLevel && runConfig.logLevel.trim().length > 0 ? `--logLevel ${runConfig.logLevel}` : '';
    ...
    const submit = {
      cmdline: `${baseCommand} ${confOptions} --jars ${jars.join(',')} s3a://${runConfig.bucket}/${metalusApplication} --driverSetupClass ${runConfig.driverSetup} --applicationId ${runConfig.applicationId} ${logOptions}`,
