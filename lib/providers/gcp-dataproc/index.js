...
class DataprocProvider extends BaseProvider {
  ...
  extractCredentials(providerInstance, user) {
    ...
    return { jsonKey: JSON.parse(credentials) };
  }
  ...
  async getNewClusterForm(providerInstance, user) {
    ...
    // TODO Pull the list of zones
    const networks = await compute.getSubnetworks();
    const subnetworksList = [];
    networks[0].forEach(n => subnetworksList.push({id: n.id, name: n.name}));
    ...
    const networkConfig = clusterForm.find(obj => obj.key === 'config.gceClusterConfig.subNetworkUri');
    networkConfig.templateOptions.options = subnetworksList;
  ...
  async createCluster(clusterConfig, providerInstance, user) {
    ...
    if (!clusterConfig.config.gceClusterConfig) {
      clusterConfig.config.gceClusterConfig = {};
    }
    clusterConfig.config.gceClusterConfig.serviceAccountScopes = [
    ...
    if (clusterConfig.useServiceAccount) {
      ...
      clusterConfig.config.gceClusterConfig.serviceAccountScopes.push('https://www.googleapis.com/auth/cloud-platform');
    ...
    // TODO This may not be needed, but we may need to add the zoneURI
    // if (clusterConfig.config.gceClusterConfig.subNetworkUri) {
    //   clusterConfig.config.gceClusterConfig.subNetworkUri = `https://www.googleapis.com/compute/v1/projects/${providerInstance.projectId}/regions/${providerInstance.region}/subnetworks/${clusterConfig.config.gceClusterConfig.subNetworkUri}`;
    // }
  ...
  async executeApplication(providerInstance, user, runConfig) {
    const creds = this.extractCredentials(providerInstance, user);
