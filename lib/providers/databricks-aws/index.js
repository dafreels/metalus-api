...
class DatabricksProvider extends AWSProvider {
  ...
  async startCluster(clusterId, clusterName, providerInstance, user) {
    const client = this.generateClusterClient(providerInstance, user.secretKey);
  ...
  async createCluster(clusterConfig, providerInstance, user) {
    ...
    try {
      ...
      const cluster = await client.get(`2.0/clusters/get?cluster_id=${clusterId}`);
      return {
  ...
  async executeApplication(providerInstance, user, runConfig) {
    ...
    const config = {
      ...
      libraries: runConfig.jars.map(j => {
        return {jar: `s3a://${runConfig.bucket}/${j}`};
      }),
      spark_jar_task: {
    ...
    if (runConfig.customFormValues && runConfig.customFormValues.useNewCluster) {
      ...
      config.new_cluster = this.addRequiredAWSAttributes(runConfig.new_cluster);
    }
    const response = await client.post(`/api/2.0/runs/submit`, config);
  ...
  async populateFormOptions(providerInstance, user, sparkVersionKey, nodeTypeKey, driverTypeKey, form) {
    ...
    if (versionsResponse.data.versions) {
      versionsResponse.data.versions.forEach((version) => {
        versionList.push({
          key: version.key,
          name: version.name
        });
    ...
    if (nodesResponse.data.node_types) {
      nodesResponse.data.node_types.forEach((node) => {
        nodeTypeList.push({
