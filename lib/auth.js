...
const getLocalStrategy = () => {
  return new LocalStrategy(async (username, password, done) => {
    ...
    const passwordResult = MetalusUtils.verifyPassword(password, user);
    if (passwordResult.match) {
      const userResult = MetalusUtils.handleEncryptionKeyChange(passwordResult, password, user);
      if (userResult.updateUser) {
        await userModel.update(user.id, user, null);
      }
      // Decrypt the secret key for the session user
      user.secretKey = MetalusUtils.decryptString(user.secretKey, userResult.encryptKey);
