      ...
      this.find(key)
        .then(results => {
          if (results.length > 0) {
            ...
            return this.updateRecord(key, record).then((newRecord) => resolve(newRecord));
          } else {
  ...
  updateRecord(key, record) {
    return new Promise((resolve, reject) => {
      this.find(key)
        .then(stored => {
          if (stored.length === 0) {
            return this.addRecord(key, record)
              .then((newRecord) => {
                resolve(newRecord);
              });
            ...
            reject('multiple records found while attempting to update');
          }
        })
        .catch(err => reject(err));
    });
